# Testing Toolkits for Mazze

Ensuring the correctness of a blockchain system like Mazze is a challenging
task. This repository comes with several rigorous
testing tools and scripts. This repository is a submodule of `Mazze-Rust` at `tests/extra-test-toolkits`. 
...
implementation of the Mazze DAG-Embedded Tree Structure (DETS) consensus algorithm together with a
random graph generator that generates random DAG-Embedded Tree Structure (DETS) blocks in a special
format. `consensus_bench` in the `Mazze-Rust` repository is capable of processing this input format, run the
Mazze consensus, and compare the results with the slow C++ implementation.
...
`tests/extra-test-toolkits/mazze_tracing.py` is a random testing script with the failure
injection capability. It will start a Mazze network with a fixed number of
...
have the consensus for the DAG-Embedded Tree Structure (DETS) and block state. To run Mazze tracing,
you need to first compile the release version of the Mazze Rust implementation
from the source code. Then you can invoke the script as follows:

```bash
$ tests/extra-test-toolkits/mazze_tracing.py run
```
...
same directory provide an automatic deployment of Mazze network on AWS for
...
3. Decide the branch of the Mazze repo you want to test. Note that this
script pulls the source code from a GitHub repo that contains the Mazze rust
implementation and compile them on the fly. You cannot run your local Mazze
copy with this script. If you do not specify the repo/branch name, it will pull
from the official Mazze-rust repo from the GitHub.
...
The storage layer in Mazze is often the performance bottleneck.
...
1. From the AWS S3 `mazze-storage-bench` bucket, download `foundation.json`
...
thousand blocks per second in the normal scenarios. However, if the DAG-Embedded Tree Structure (DETS)
...
1. `fork_same_height_merge.py` creates a unstable DAG-Embedded Tree Structure (DETS) with roughly 95000
blocks. In the DAG-Embedded Tree Structure (DETS), it has three branches and in each branch there are
