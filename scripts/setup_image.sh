...
branch=${1:-master}
repo="${2:-https://github.com/Mazze-Chain/mazze-rust}"
...
if [[ ! -d mazze-rust ]]; then
  git clone https://github.com/Mazze-Chain/mazze-rust
fi

cd mazze-rust
git reset --hard
if [ $repo == "https://github.com/Mazze-Chain/mazze-rust" ]; then
    repo="origin"
else
...
wget https://s3-ap-southeast-1.amazonaws.com/mazze-test/genesis_secrets.txt
cp ../../../target/release/mazze throttle_bitcoin_bandwidth.sh remote_start_mazze.sh remote_collect_log.sh stat_latency_map_reduce.py genesis_secrets.txt ~
