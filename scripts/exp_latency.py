...
import argparse
from remote_simulate import RemoteSimulate, pssh, kill_remote_mazze, execute
import subprocess
from test_framework.test_framework import OptionHelper

def cleanup_remote_logs(ips_file:str):
    pssh(ips_file, "rm -f *.tgz *.out; rm -rf /tmp/mazze_test_*")
...
class LatencyExperiment:
    def __init__(self):
        ...
        remote_simulate_options.update(RemoteSimulate.PASS_TO_MAZZE_OPTIONS)
    ...
    def run(self):
        for config in RemoteSimulateConfig.parse(self.options.batch_config):
            ...
            print("kill remote mazze and cleanup logs ...")
            ...
            print("Kill remote mazze and copy logs ...")
            kill_remote_mazze(self.options.ips_file)
            ...
            execute("./copy_file_from_slave.sh mazze.log {} > /dev/null".format(tag), 3, "collect rust log")
            if self.options.enable_flamegraph:
                try:
                    execute("./copy_file_from_slave.sh mazze.svg {} > /dev/null".format(tag), 10, "collect flamegraph")
                except:
                    print("Failed to copy flamegraph file mazze.svg, please try again via copy_file_from_slave.sh in manual")
        ...
        cmd = "tar cvfz {} {} *.exp.log *nodes.csv *.metrics.log *.mazze.log".format(self.stat_archive_file, self.stat_log_file)
        if self.options.enable_flamegraph:
            cmd = cmd + " *.mazze.svg"
        os.system(cmd)
