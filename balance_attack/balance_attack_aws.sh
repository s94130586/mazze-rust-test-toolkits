...
branch="${4:-master}"
repo="${5:-https://github.com/Mazze-Chain/mazze-rust}"
...
run_balance_attack () {
    ...
    ssh ubuntu@${master_ip} "cd ./mazze-rust/tests/extra-test-toolkits/scripts;
    ...
    ssh ubuntu@${master_ip} "cd ./mazze-rust/tests/extra-test-toolkits/scripts;
      ...
      parallel-scp -O \"StrictHostKeyChecking no\" -h ips -l \
        ubuntu -p 1000 ../../../target/release/mazze ~ |grep FAILURE|wc -l;"
    ...
    archive_file="mazze_logs.tgz"
    parallel-ssh -O "StrictHostKeyChecking no" -h ips -p 400 -t 600 "tar czvf \"$archive_file\" /tmp/mazze_test_*/*log /tmp/mazze_test_*/std*"
