...
from mazze.utils import encode_hex, bytes_to_int, priv_to_addr, parse_as_int, pub_to_addr
from test_framework.test_framework import MazzeTestFramework, OptionHelper
...
def kill_remote_mazze(ips_file:str):
    pssh(ips_file, "killall mazze || echo already killed", 3, "kill remote mazze")

"""
Setup and run mazze nodes on multiple vms with a few nodes on each vm.
"""
class RemoteSimulate(MazzeTestFramework):
    ...
    PASS_TO_MAZZE_OPTIONS = dict(
        ...
        tx_pool_size = mazze.config.default_mazze_conf["tx_pool_size"],
        max_block_size_in_bytes = mazze.config.default_config["MAX_BLOCK_SIZE_IN_BYTES"],
        # pos
        ...
        public_rpc_apis="mazze,debug,test,pubsub,trace"
    )

    def add_options(self, parser:ArgumentParser):
        OptionHelper.add_options(parser, RemoteSimulate.SIMULATE_OPTIONS)
        OptionHelper.add_options(parser, RemoteSimulate.PASS_TO_MAZZE_OPTIONS)

    def after_options_parsed(self):
        MazzeTestFramework.after_options_parsed(self)
        ...
        self.conf_parameters = OptionHelper.mazze_options_to_config(
            vars(self.options), RemoteSimulate.PASS_TO_MAZZE_OPTIONS)

        # Default Mazze memory consumption
        target_memory = 16
        # Overwrite with scaled configs so that Mazze consumes storage_memory_gb rather than target_memory.
        for k in ["db_cache_size", "ledger_cache_size",
            ...
            self.conf_parameters[k] = str(
                mazze.config.production_conf[k] // target_memory * self.options.storage_memory_gb)
    ...
    def stop_nodes(self):
        kill_remote_mazze(self.options.ips_file)

    def setup_remote_mazze(self):
        # tar the config file for all nodes
        zipped_conf_file = os.path.join(self.options.tmpdir, "mazze_conf.tgz")
        ...
        self.log.info("copy mazze configuration files to remote nodes ...")
        pscp(self.options.ips_file, zipped_conf_file, "~", 3, "copy mazze configuration files to remote nodes")
        os.remove(zipped_conf_file)

        # setup on remote nodes and start mazze
        self.log.info("setup mazze runtime environment and start mazze on remote nodes ...")
        cmd_kill_mazze = "killall -9 mazze || echo already killed"
        cmd_cleanup = "rm -rf /tmp/mazze_test_*"
        cmd_setup = "tar zxf mazze_conf.tgz -C /tmp"
        cmd_startup = "./remote_start_mazze.sh {} {} {} {} {}&> start_mazze.out".format(
        ...
        cmd = "{}; {} && {} && {}".format(cmd_kill_mazze, cmd_cleanup, cmd_setup, cmd_startup)
        pssh(self.options.ips_file, cmd, 3, "setup and run mazze on remote nodes")

    def setup_network(self):
        self.setup_remote_mazze()
    ...
    def gather_confirmation_latency_async(self):
        ...
        def get_risk(block):
            ...
            risk = self.nodes[p].mazze_getConfirmationRiskByHash(block)
    ...
    def run_test(self):
        ...
        # When enable_tx_propagation is set, let mazze nodes generate tx automatically.
        self.init_txgen()
        ...
        for i in range(1, node0.epoch_number()+1):
            main_block = node0.block_by_epoch(node0.EPOCH_NUM(i))["hash"]
            if main_block in self.confirm_info.block_confirmation_time:
                ghost_confirmation_time.append(self.confirm_info.block_confirmation_time[main_block])
